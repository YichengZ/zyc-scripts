...
local layer_files = {
  table      = "panda_base_desk.png",
  head       = "panda_base_head.png",
  face       = "panda_base_face.png",
  rest       = "panda_base_rest.png",
  hand_left  = "panda_base_hand_left.png",
  hand_right = "panda_base_hand_right.png",
}
...
local function load_layer(name, file)
  layers[name] = try_load_image(root_path .. "assets/skins/panda_base/" .. file)
end

local function load_legacy_full()
  legacy_full =
    try_load_image(root_path .. "assets/skins/panda_base/panda_base.png") or
    try_load_image(root_path .. "assets/skins/panda_base/panda_base.jpg")
end
...
local function TriggerTap()
  ...
  if state.next_hand_is_left then
    ...
    SpawnParticle(
      state.panda_cx + particle_config.left_offset_x * s,
      state.panda_cy + particle_config.offset_y * s
    )
  else
    state.right_target = tap_amplitude
    SpawnParticle(
      state.panda_cx + particle_config.right_offset_x * s,
      state.panda_cy + particle_config.offset_y * s
    )
  end
...
local function update_anim(ctx, char_state)
  ...
  if char_state == "break" then
    ...
    local s = state.scale
    local face_center_x = state.panda_cx
    local face_center_y = state.panda_cy - 25 * s
...
function PandaBase.init(ctx, script_path)
  ctx_ref = ctx
...
function PandaBase.update(dt, char_state, ctx)
...
local function draw_composed_layers(dl, rect, char_state, skip_overlay)
  ...
  state.scale = scale
  state.panda_cx = rect.center_x
  state.panda_cy = rect.center_y
...
function PandaBase.draw(ctx, dl, x, y, w, h, char_state, skip_overlay)
...
function PandaBase.draw_paws(ctx, dl, x, y, w, h)
...
function PandaBase.get_draw_rect() return state.last_rect end
function PandaBase.get_recommended_size() return 300, 200 end

function PandaBase.trigger_action(action_type, is_manual)
  ...
  elseif action_type == "celebrate" then
    local s = state.scale
    SpawnParticle(state.panda_cx + particle_config.left_offset_x * s, state.panda_cy + particle_config.offset_y * s)
    SpawnParticle(state.panda_cx + particle_config.right_offset_x * s, state.panda_cy + particle_config.offset_y * s)
  end
end

function PandaBase.get_particle_config() return particle_config end
function PandaBase.set_particle_config(new_config)
  if new_config then
...
function PandaBase.get_zzz_config() return zzz_config end
function PandaBase.set_zzz_config(new_config)
  if new_config then
    for k,v in pairs(new_config) do zzz_config[k] = v end
  end
end

function PandaBase.get_last_manual_tap_time() return state.last_manual_tap_time or 0 end
