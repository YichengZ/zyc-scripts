...
local layer_files = {
  table      = "dog_base_desk.png",
  head       = "dog_base_head.png",
  face       = "dog_base_face.png",
  rest       = "dog_base_rest.png",
  hand_left  = "dog_base_hand_left.png",
  hand_right = "dog_base_hand_right.png",
}
...
local function load_layer(name, file)
  layers[name] = try_load_image(root_path .. "assets/skins/dog_base/" .. file)
end

local function load_legacy_full()
  legacy_full =
    try_load_image(root_path .. "assets/skins/dog_base/dog_base.png") or
    try_load_image(root_path .. "assets/skins/dog_base/dog_base.jpg")
end
...
local function TriggerTap()
  ...
  if state.next_hand_is_left then
    ...
    SpawnParticle(
      state.dog_cx + particle_config.left_offset_x * s,
      state.dog_cy + particle_config.offset_y * s
    )
  else
    state.right_target = tap_amplitude
    SpawnParticle(
      state.dog_cx + particle_config.right_offset_x * s,
      state.dog_cy + particle_config.offset_y * s
    )
  end
...
local function update_anim(ctx, char_state)
  ...
  if char_state == "break" then
    ...
    local s = state.scale
    local face_center_x = state.dog_cx
    local face_center_y = state.dog_cy - 25 * s
...
function DogBase.init(ctx, script_path)
  ctx_ref = ctx
...
function DogBase.update(dt, char_state, ctx)
...
local function draw_composed_layers(dl, rect, char_state, skip_overlay)
  ...
  state.scale = scale
  state.dog_cx = rect.center_x
  state.dog_cy = rect.center_y
...
function DogBase.draw(ctx, dl, x, y, w, h, char_state, skip_overlay)
...
function DogBase.draw_paws(ctx, dl, x, y, w, h)
...
function DogBase.get_draw_rect() return state.last_rect end
function DogBase.get_recommended_size() return 300, 200 end

function DogBase.trigger_action(action_type, is_manual)
  ...
  elseif action_type == "celebrate" then
    local s = state.scale
    SpawnParticle(state.dog_cx + particle_config.left_offset_x * s, state.dog_cy + particle_config.offset_y * s)
    SpawnParticle(state.dog_cx + particle_config.right_offset_x * s, state.dog_cy + particle_config.offset_y * s)
  end
end

function DogBase.get_particle_config() return particle_config end
function DogBase.set_particle_config(new_config)
  if new_config then
...
function DogBase.get_zzz_config() return zzz_config end
function DogBase.set_zzz_config(new_config)
  if new_config then
    for k,v in pairs(new_config) do zzz_config[k] = v end
  end
end

function DogBase.get_last_manual_tap_time() return state.last_manual_tap_time or 0 end
