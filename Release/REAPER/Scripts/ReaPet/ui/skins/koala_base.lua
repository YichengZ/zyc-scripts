--[[
  Koala Base PNG 皮肤（性能优化版）
  结构与 cat_base/onion_base 一致，使用 PNG 分层渲染。
--]]
...
local layer_files = {
  table      = "koala_base_desk.png",
  head       = "koala_base_head.png",
  face       = "koala_base_face.png",
  rest       = "koala_base_rest.png",
  hand_left  = "koala_base_hand_left.png",
  hand_right = "koala_base_hand_right.png",
}
...
local function load_layer(name, file)
  layers[name] = try_load_image(root_path .. "assets/skins/koala_base/" .. file)
end

local function load_legacy_full()
  legacy_full =
    try_load_image(root_path .. "assets/skins/koala_base/koala_base.png") or
    try_load_image(root_path .. "assets/skins/koala_base/koala_base.jpg")
end
...
local function TriggerTap()
  ...
  if state.next_hand_is_left then
    ...
    SpawnParticle(
      state.koala_cx + particle_config.left_offset_x * s,
      state.koala_cy + particle_config.offset_y * s
    )
  else
    state.right_target = tap_amplitude
    SpawnParticle(
      state.koala_cx + particle_config.right_offset_x * s,
      state.koala_cy + particle_config.offset_y * s
    )
  end
...
local function update_anim(ctx, char_state)
  ...
  if char_state == "break" then
    ...
    local s = state.scale
    local face_center_x = state.koala_cx
    local face_center_y = state.koala_cy - 25 * s
...
function KoalaBase.init(ctx, script_path)
  ctx_ref = ctx
...
function KoalaBase.update(dt, char_state, ctx)
...
local function draw_composed_layers(dl, rect, char_state, skip_overlay)
  ...
  state.scale = scale
  state.koala_cx = rect.center_x
  state.koala_cy = rect.center_y
...
function KoalaBase.draw(ctx, dl, x, y, w, h, char_state, skip_overlay)
...
function KoalaBase.draw_paws(ctx, dl, x, y, w, h)
...
function KoalaBase.get_draw_rect() return state.last_rect end
function KoalaBase.get_recommended_size() return 300, 200 end

function KoalaBase.trigger_action(action_type, is_manual)
  ...
  elseif action_type == "celebrate" then
    local s = state.scale
    SpawnParticle(state.koala_cx + particle_config.left_offset_x * s, state.koala_cy + particle_config.offset_y * s)
    SpawnParticle(state.koala_cx + particle_config.right_offset_x * s, state.koala_cy + particle_config.offset_y * s)
  end
end

function KoalaBase.get_particle_config() return particle_config end
function KoalaBase.set_particle_config(new_config)
  if new_config then
...
function KoalaBase.get_zzz_config() return zzz_config end
function KoalaBase.set_zzz_config(new_config)
  if new_config then
    for k,v in pairs(new_config) do zzz_config[k] = v end
  end
end

function KoalaBase.get_last_manual_tap_time() return state.last_manual_tap_time or 0 end
