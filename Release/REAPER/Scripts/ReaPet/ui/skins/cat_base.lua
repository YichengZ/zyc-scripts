...
local layer_files = {
  table      = "cat_base_desk.png",
  head       = "cat_base_head.png",
  face       = "cat_base_face.png",
  rest       = "cat_base_rest.png",
  hand_left  = "cat_base_hand_left.png",
  hand_right = "cat_base_hand_right.png",
}
...
local function load_layer(name, file)
  layers[name] = try_load_image(root_path .. "assets/skins/cat_base/" .. file)
end

local function load_legacy_full()
  legacy_full =
    try_load_image(root_path .. "assets/skins/cat_base/cat_base.png") or
    try_load_image(root_path .. "assets/skins/cat_base/cat_base.jpg")
end
...
local function TriggerTap()
  ...
  if state.next_hand_is_left then
    ...
    SpawnParticle(
      state.cat_cx + particle_config.left_offset_x * s,
      state.cat_cy + particle_config.offset_y * s
    )
  else
    state.right_target = tap_amplitude
    SpawnParticle(
      state.cat_cx + particle_config.right_offset_x * s,
      state.cat_cy + particle_config.offset_y * s
    )
  end
...
local function update_anim(ctx, char_state)
  ...
  if char_state == "break" then
    ...
    local s = state.scale
    local face_center_x = state.cat_cx
    local face_center_y = state.cat_cy - 25 * s
...
function CatBase.init(ctx, script_path)
  ctx_ref = ctx
...
function CatBase.update(dt, char_state, ctx)
...
local function draw_composed_layers(dl, rect, char_state, skip_overlay)
  ...
  state.scale = scale
  state.cat_cx = rect.center_x
  state.cat_cy = rect.center_y
...
function CatBase.draw(ctx, dl, x, y, w, h, char_state, skip_overlay)
...
function CatBase.draw_paws(ctx, dl, x, y, w, h)
...
function CatBase.get_draw_rect() return state.last_rect end
function CatBase.get_recommended_size() return 300, 200 end

function CatBase.trigger_action(action_type, is_manual)
  ...
  elseif action_type == "celebrate" then
    local s = state.scale
    SpawnParticle(state.cat_cx + particle_config.left_offset_x * s, state.cat_cy + particle_config.offset_y * s)
    SpawnParticle(state.cat_cx + particle_config.right_offset_x * s, state.cat_cy + particle_config.offset_y * s)
  end
end

function CatBase.get_particle_config() return particle_config end
function CatBase.set_particle_config(new_config)
  if new_config then
...
function CatBase.get_zzz_config() return zzz_config end
function CatBase.set_zzz_config(new_config)
  if new_config then
    for k,v in pairs(new_config) do zzz_config[k] = v end
  end
end

function CatBase.get_last_manual_tap_time() return state.last_manual_tap_time or 0 end
