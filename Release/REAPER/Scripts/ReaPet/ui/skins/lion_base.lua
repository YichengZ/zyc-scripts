...
local layer_files = {
  table      = "lion_base_desk.png",
  head       = "lion_base_head.png",
  face       = "lion_base_face.png",
  rest       = "lion_base_rest.png",
  hand_left  = "lion_base_hand_left.png",
  hand_right = "lion_base_hand_right.png",
}
...
local function load_layer(name, file)
  layers[name] = try_load_image(root_path .. "assets/skins/lion_base/" .. file)
end

local function load_legacy_full()
  legacy_full =
    try_load_image(root_path .. "assets/skins/lion_base/lion_base.png") or
    try_load_image(root_path .. "assets/skins/lion_base/lion_base.jpg")
end
...
local function TriggerTap()
  ...
  if state.next_hand_is_left then
    ...
    SpawnParticle(
      state.lion_cx + particle_config.left_offset_x * s,
      state.lion_cy + particle_config.offset_y * s
    )
  else
    state.right_target = tap_amplitude
    SpawnParticle(
      state.lion_cx + particle_config.right_offset_x * s,
      state.lion_cy + particle_config.offset_y * s
    )
  end
...
local function update_anim(ctx, char_state)
  ...
  if char_state == "break" then
    ...
    local s = state.scale
    local face_center_x = state.lion_cx
    local face_center_y = state.lion_cy - 25 * s
...
function LionBase.init(ctx, script_path)
  ctx_ref = ctx
...
function LionBase.update(dt, char_state, ctx)
...
local function draw_composed_layers(dl, rect, char_state, skip_overlay)
  ...
  state.scale = scale
  state.lion_cx = rect.center_x
  state.lion_cy = rect.center_y
...
function LionBase.draw(ctx, dl, x, y, w, h, char_state, skip_overlay)
...
function LionBase.draw_paws(ctx, dl, x, y, w, h)
...
function LionBase.get_draw_rect() return state.last_rect end
function LionBase.get_recommended_size() return 300, 200 end

function LionBase.trigger_action(action_type, is_manual)
  ...
  elseif action_type == "celebrate" then
    local s = state.scale
    SpawnParticle(state.lion_cx + particle_config.left_offset_x * s, state.lion_cy + particle_config.offset_y * s)
    SpawnParticle(state.lion_cx + particle_config.right_offset_x * s, state.lion_cy + particle_config.offset_y * s)
  end
end

function LionBase.get_particle_config() return particle_config end
function LionBase.set_particle_config(new_config)
  if new_config then
...
function LionBase.get_zzz_config() return zzz_config end
function LionBase.set_zzz_config(new_config)
  if new_config then
    for k,v in pairs(new_config) do zzz_config[k] = v end
  end
end

function LionBase.get_last_manual_tap_time() return state.last_manual_tap_time or 0 end
